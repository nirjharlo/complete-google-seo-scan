<?php
 ...
 $cgss_generated_content_result = __( 'Following is the content which can be craweled without blocking. This is not seen as auto generated.', 'cgss' ) . '<br />' . __( 'You can compare your actual page ( in your browser ) with this content to check if javascripts have generated more contents.', 'cgss' ) . '<br /><a id="pagecontent">' . __( 'Page content &darr;', 'cgss' ) . '</a>';
 $cgss_generated_content_action = __( 'If you don\'t see excess content no action is required', 'cgss' ) . '<br />' . __( 'Otherwise remove those content generating javascripts.', 'cgss' );
 ...
 //meta open graph protocol results
 ...
 if ( array_key_exists( 'Cache-Control', $cgss_headers ) ) {
  ...
  $cgss_caching_action = __( 'Use cache method for better user experience. Use any free cache plugin available.', 'cgss' );
 }
 ...
 if ( substr( $cgss_gzip_header_data, 0, 4 ) == 'gzip' ) {
  $cgss_gzip_pointer = 0;
  $cgss_gzip_result = __( 'We found gzip compression in your web page headers. It\'s great.', 'cgss' );
  $cgss_gzip_action = __( 'No action required.', 'cgss' );
 } else {
  $cgss_gzip_pointer = 1;
  $cgss_gzip_result = __( 'No compression method is found in web page.', 'cgss' );
  $cgss_gzip_action = __( 'Enable gzip compression for better results.', 'cgss' );
 }
 ...
 $cgss_total_pointer = $cgss_title_pointer + $cgss_image_pointer + $cgss_head_tag_pointer + $cgss_link_pointer + $cgss_link_attr_pointer + $cgss_html_text_pointer + $cgss_url_pointer + $cgss_http_reqests_pointer + $cgss_meta_desc_pointer + $cgss_meta_cano_pointer + $cgss_meta_robots_pointer + $cgss_meta_ogp_pointer + $cgss_if_mod_since_pointer + $cgss_robot_command_pointer + $cgss_sitemap_pointer + $cgss_cms_pointer + $cgss_caching_pointer + $cgss_keep_alive_pointer + $cgss_gzip_pointer;
 $cgss_success_prcentage = round( ( ( $cgss_total_pointer / 21 ) * 100 ), 2 );
 ...
 $cgss_scaned_page_props = array(
	...
	array( __( 'General Result', 'cgss' ), $cgss_success_prcentage . '&nbsp;%&nbsp;' . __( 'successful in passing all tests.', 'cgss' ) ),
 ...
 $cgss_tech_results = array (
  ...
  array( __( 'gzip compression', 'cgss' ), __( 'This server feature delivers web page content to browser in gzip compressed format. Hence it lowers bandwidth and web page size.', 'cgss' ), $keep_alive_result, $keep_alive_action ),
);
?>
